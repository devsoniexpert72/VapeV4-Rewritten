...
run(function()
    ...
    local buildSpeedSlider, maxModeToggle
    ...
    -- Utility: Strict Block Checking for Scaling
    ...
    -- =========================================================================
    local function ParseVec(vecStr, def)
        if type(vecStr) == "table" then return vecStr end
        if not vecStr then return def end
        local x, y, z = string.match(vecStr, "([%d.-]+)%s*,%s*([%d.-]+)%s*,%s*([%d.-]+)")
        return {tonumber(x) or def[1], tonumber(y) or def[2], tonumber(z) or def[3]}
    end
    ...
    local function normalizeBuildData(rawData)
        ...
        if autoDetectToggle.Enabled then
            if type(rawData) == "table" and rawData[1] and type(rawData[1]) == "table" and rawData[1].Type then
                formatType = "Vape AutoBuild"
            elseif type(rawData) == "table" and rawData[1] and type(rawData[1][1]) == "string" then
        ...
        -- Return if already native
        if formatType == "Vape AutoBuild" then return rawData end

        local normalized = {}
        local dict = (formatType == "Luau Pro Format") and rawData[2] or rawData
        
        -- Convert Luau/Maxima dictionaries into Vape Array Format
            ...
            if type(list) == "table" then
                for _, bData in ipairs(list) do
                    ...
                    table.insert(normalized, {
                        ...
                        Color = ParseVec(bData.Color, {1,1,1}),
                        Props = props
                    })
                end
            end
        ...
        for _, pData in ipairs(previewParts) do
            if pData.part and pData.part.Parent then
                ...
                if pData.part:IsA("Model") then
                    pData.part:PivotTo(absoluteCF)
                else
                    pData.part.CFrame = absoluteCF
                end
            end
        end
    end
    ...
    previewBuilderModule = vape.Categories['BABFT Tools']:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                local scaleMultiplier = scaleSlider.Value / 10 -- Convert 10 to 1.0x
...
run(function()
    ...
    local previewFolderName = "Video_Preview_Hologram"
    local cachedVideoData = nil
    local urlCache = {} -- Smart Caching System
    
    -- Playback Globals
    local isPlaying = false
    local isGridGenerated = false
    local screenBlocks = {} 
    local currentColors = {}
    ...
    -- =========================================================================
    -- HOLOGRAM PREVIEW GENERATOR
    -- =========================================================================
    local function GeneratePreviewHologram()
        if not cachedVideoData then return end
        local plotZone = getPlotZone()
        if not plotZone then return end
        ...
        local sizeMult = partSizeSlider.Value / 10
        local surfaceY = (plotZone.Size.Y / 2) + (sizeMult / 2)
        local halfWidth = (cachedVideoData.width * sizeMult) / 2
        local baseOffset = CFrame.new(offX.Value, offY.Value, offZ.Value)
        local targetBaseCF = plotZone.CFrame * baseOffset

        local firstFrame = cachedVideoData.frames[1]
        local i = 1
        
        for y = 0, cachedVideoData.height - 1 do
            for x = 0, cachedVideoData.width - 1 do
                local colorData = firstFrame[i]
                if colorData then
                    local ghost = Instance.new("Part")
                    ghost.Anchored = true
                    ghost.CanCollide = false
                    ...
                    ghost.Transparency = 0.5
                    ghost.Color = Color3.new(unpack(colorData))
                    ghost.Size = Vector3.new(sizeMult, sizeMult, sizeMult)
                    
                    local rY = ((cachedVideoData.height - y - 1) * sizeMult) + surfaceY
                    local rX = (x * sizeMult) - halfWidth
                    
                    ghost.CFrame = targetBaseCF:ToWorldSpace(CFrame.new(rX, rY, 0))
                    ghost.Parent = previewFolder
                end
                i = i + 1
            end
    ...
    -- =========================================================================
    -- MAIN PLAYBACK MODULE (TOGGLE)
    -- =========================================================================
    videoModule = vape.Categories['BABFT Tools']:CreateModule({
        Name = 'Video & GIF Player',
        Tooltip = 'Plays the video on the pre-existing grid.',
        Function = function(callback)
            ...
            task.spawn(function()
                if not cachedVideoData or not cachedVideoData.frames then
                    notify('Error', 'No video processed! Fetch it first.', 5, 'alert')
                    videoModule:Toggle() return
                end
                
                if not isGridGenerated or #screenBlocks == 0 then
                    notify('Error', 'Generate grid before playing!', 5, 'alert')
                    videoModule:Toggle() return
                end

                local char = lp.Character or lp.CharacterAdded:Wait()
                local paintTool = lp.Backpack:FindFirstChild("PaintingTool") or char:FindFirstChild("PaintingTool")

                if not paintTool then
                    notify('Error', 'Missing PaintingTool in inventory.', 5, 'alert')
                    videoModule:Toggle() return
                end

                notify('Video Player', 'Playback started!', 3, 'info')

                -- ANIMATION LOOP (Differential Painting - Single List)
                isPlaying = true
                while videoModule.Enabled and isPlaying do
                    
                    for f_idx = 1, cachedVideoData.total_frames do
                        if not videoModule.Enabled or not isPlaying then return end
                        
                        local frame = cachedVideoData.frames[f_idx]
                        local paintArgs = {}
                        
                        for idx, block in pairs(screenBlocks) do
                            if block and block.Parent then
                                local targetColor = Color3.new(unpack(frame[idx]))
                                -- DIFFERENTIAL OPTIMIZATION: Only add if changed
    ...
    -- =========================================================================
    videoModule:CreateButton({
        Name = 'Fetch & Preview First Frame',
        Function = function()
            local url = linkBox.Value
            ...
            task.spawn(function()
                local resLevel = resSlider.Value
                local cacheKey = url .. "_res" .. resLevel
                
                -- Check Local Cache first
                if urlCache[cacheKey] then
                    cachedVideoData = urlCache[cacheKey]
                    fpsSlider:SetValue(cachedVideoData.fps)
                    notify('Cache Loaded', 'Video loaded instantly from cache!', 5, 'info')
                    GeneratePreviewHologram()
                    return
                end

                notify('Processing', 'Asking Python Server to process Video...', 3, 'info')
                
                local qualityMap = { [1] = 40, [2] = 64, [3] = 100, [4] = 160 }
                local targetWidth = qualityMap[resLevel]
                local fetchUrl = "http://localhost:8000/video?width=" .. targetWidth .. "&url=" .. HttpService:UrlEncode(url)
                ...
                cachedVideoData = decoded
                urlCache[cacheKey] = decoded -- Save to cache
                fpsSlider:SetValue(decoded.fps)
                
                notify('Success', 'Video mapped! (' .. decoded.total_frames .. ' frames). Requires ' .. (decoded.width * decoded.height) .. ' blocks.', 10, 'info')
    ...
    -- =========================================================================
    -- GENERATE CANVAS GRID BUTTON
    -- =========================================================================
    videoModule:CreateButton({
        Name = 'Generate Canvas Grid',
        Function = function()
            if not cachedVideoData then
                notify('Error', 'Fetch a video first before generating the grid.', 5, 'alert')
                return
            end
            
            task.spawn(function()
                isGridGenerated = false
                table.clear(screenBlocks)
                table.clear(currentColors)
                ...
                local totalBlocks = cachedVideoData.width * cachedVideoData.height
                notify('Grid Generator', 'Building ' .. totalBlocks .. ' block canvas...', 5, 'info')
                ...
                while threadsCompleted < totalBlocks do task.wait() end
                blockAddedConn:Disconnect()
                
                -- Paint the first frame as a "Paused Video" aesthetic
                if paintTool and #paintArgs > 0 then
                    task.spawn(function() paintTool:WaitForChild("RF"):InvokeServer(paintArgs) end)
                end

                isGridGenerated = true
                notify('Grid Generated', 'Canvas complete! Waiting 5 seconds before playback unlocks...', 5, 'info')
                
                task.wait(5)
                notify('Ready', 'You can now toggle the module ON to start the video.', 5, 'info')
            end)
        end
    })
    ...
    -- UI COMPONENTS
    -- =========================================================================
    linkBox = videoModule:CreateTextBox({ Name = 'Video / GIF Link (MP4)', Default = "", Tooltip = "Direct URL to an .mp4, .webm, or .gif file."})
    
    resSlider = videoModule:CreateSlider({ Name = 'Video Quality Profile (144p -> 480p)', Min = 1, Max = 4, Default = 2, Function = GeneratePreviewHologram, Tooltip = "Higher settings require exponentially more blocks. 4 = 160px wide (Requires 10k+ blocks)." })
    partSizeSlider = videoModule:CreateSlider({ Name = 'Pixel Scale Multiplier (x10)', Min = 1, Max = 100, Default = 10, Function = GeneratePreviewHologram })
    fpsSlider = videoModule:CreateSlider({ Name = 'Playback FPS', Min = 1, Max = 60, Default = 15, Function = function() end, Tooltip = "Adjusts playback speed. Automatically sets on fetch." })
    
    loopToggle = videoModule:CreateToggle({ Name = 'Loop Playback', Default = true, Function = function() end })

    blockDropdown = videoModule:CreateDropdown({ Name = 'Canvas Block Type', List = getAvailableBlocks(), Function = function() end})
    fallbackBox = videoModule:CreateTextBox({ Name = 'Fallback Blocks (Comma Sep)', Default = "PlasticBlock, WoodBlock", Function = function() end })
    
    offX = videoModule:CreateSlider({ Name = 'Offset X', Min = -1000, Max = 1000, Default = 0, Function = GeneratePreviewHologram })
    offY = videoModule:CreateSlider({ Name = 'Offset Y', Min = -1000, Max = 1000, Default = 100, Function = GeneratePreviewHologram })
    offZ = videoModule:CreateSlider({ Name = 'Offset Z', Min = -1000, Max = 1000, Default = 0, Function = GeneratePreviewHologram })

    speedSlider = videoModule:CreateSlider({ Name = 'Canvas Spawn Speed', Min = 100, Max = 1000, Default = 500, Function = function() end })
    maxModeToggle = videoModule:CreateToggle({ Name = 'Max Mode (Lag Warning)', Default = false, Function = function(val) if val then notify('Warning', 'Max mode may lag or crash.', 5, 'alert') end end })
...
run(function()
    ...
    local function getPlayerNames(includeAll)
        local names = includeAll and {"All"} or {}
    ...
    local function executeBring(targetPlayer)
        ...
        if not playerBlocksFolder then 
            notify("FE Bring", "Your blocks folder was not found!", 5, "alert")
            return false 
        end
        ...
        local bg = Instance.new("BodyGyro", furthestSeat)
        bg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
        bg.P = 10000

        notify("FE Bring", "Attempting to capture " .. targetPlayer.Name .. "...", 3, "info")
        ...
        bringConn = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            local jitterZ = math.sin(tick() * 30) * 1.5
            
            bp.Position = tRoot.Position + Vector3.new(jitterX, jitterY, jitterZ)
            bg.CFrame = tRoot.CFrame * CFrame.Angles(math.rad(jitterX*10), math.rad(jitterY*10), math.rad(jitterZ*10))
        end)
    ...
    feBringModule = vape.Categories['Trolling']:CreateModule({
        ...
        Function = function(callback)
            ...
            task.spawn(function()
                local targetName = bringTargetDropdown.Value
                
                if targetName == "All" then
                    for _, p in ipairs(game:GetService("Players"):GetPlayers()) do
                        if p ~= lp and feBringModule.Enabled then
                            executeBring(p)
                            task.wait(0.5)
                        end
                    end
                    notify("FE Bring", "Finished looping through all players.", 5, "info")
                    feBringModule:Toggle()
                else
                    local targetPlayer = game:GetService("Players"):FindFirstChild(targetName)
                    if targetPlayer then
                        executeBring(targetPlayer)
                    else
    ...
    cannonBlastModule = vape.Categories['Trolling']:CreateModule({
        ...
        Function = function(callback)
            ...
            task.spawn(function()
                local targetName = cannonTargetDropdown.Value
                local targetPlayer = game:GetService("Players"):FindFirstChild(targetName)
                
                if not targetPlayer then
                    notify("Cannon Blast", "Target not found!", 5, "alert")
                ...
                blastConn = game:GetService("RunService").Heartbeat:Connect(function()
                    if not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if tick() - startTime > 2 then
                        ...
                        task.delay(0.5, function()
                            if cannonBlastModule.Enabled then cannonBlastModule:Toggle() end
                        end)
                        return
                    end
                ...
                cannonBlastModule:Clean(blastConn)
            end)
        end
    })
    ...
    -- =========================================================================
    -- 3. HOMING MISSILE (CINEMATIC)
    -- =========================================================================
    local homingMissileModule
    local missileTargetDropdown
    local cinematicToggle
    local crazySlider
    
    homingMissileModule = vape.Categories['Trolling']:CreateModule({
        Name = 'Homing Missile',
        Tooltip = 'Flies an unanchored Thruster 150 studs up, curves unpredictably, and hunts the target.',
        Function = function(callback)
            if not callback then return end
            ...
            task.spawn(function()
                ...
                notify("Homing Missile", "TARGET LOCKED: " .. targetName, 3, "info")
                ...
                trackerGui.Size = UDim2.fromScale(4, 1)
                trackerGui.StudsOffset = Vector3.new(0, 3, 0)
                trackerGui.AlwaysOnTop = true
                
                local trackerText = Instance.new("TextLabel")
                trackerText.Size = UDim2.fromScale(1, 1)
                trackerText.BackgroundTransparency = 1
                trackerText.TextColor3 = Color3.new(1, 0, 0)
                trackerText.TextStrokeTransparency = 0
                trackerText.Font = Enum.Font.Code
                trackerText.TextScaled = true
                trackerText.Text = "⚠️ TARGET LOCKED ⚠️"
                trackerText.Parent = trackerGui
                trackerGui.Parent = targetPlayer.Character:FindFirstChild("Head") or targetPlayer.Character:FindFirstChild("HumanoidRootPart")

                -- =====================
                -- CINEMATIC CAMERA
                -- =====================
                local cam = workspace.CurrentCamera
                local wasCinematic = cinematicToggle.Enabled
                ...
                homingMissileModule:Clean(cleanupVisuals)
                ...
                -- Turn on thruster exhaust visual
                ...
                bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                bp.P = 2000 -- Gentle initial launch
                bp.D = 300
                ...
                local phase = "Ascent"
                local ascentPos = missile.Position + Vector3.new(0, 150, 0)
                local startTime = tick()
                ...
                missileConn = game:GetService("RunService").Heartbeat:Connect(function()
                    ...
                    local tPos = targetPlayer.Character.HumanoidRootPart.Position
                    local dist = (missile.Position - tPos).Magnitude

                    if tick() - startTime > 25 then
                        notify("Missile", "Missile fuel depleted.", 5, "warning")
                        if missileConn then missileConn:Disconnect() end
                        homingMissileModule:Toggle()
                        return
                    end
                    ...
                    if phase == "Ascent" then
                        ...
                        -- Switch to homing after it gets high enough, or 3 seconds pass
                        if (missile.Position - ascentPos).Magnitude < 20 or tick() - startTime > 3 then
                            phase = "Homing"
                            bp.P = 5000 -- Increase grip for tracking
                        end
                        
                    elseif phase == "Homing" then
                        -- Collision / Hit Detection (< 6 studs guarantees an impact register)
                        if dist < 6 then
                            notify("Homing Missile", "Direct Hit!", 5, "info")
                            
                            -- Fake visual explosion for cinematic flair
                        ...
                        -- Dynamic Snake & Curve Algorithm
                        local crazy = crazySlider.Value
                        
                        -- The closer the missile gets, the tighter and more accurate the curve becomes
                        local curveMultiplier = math.clamp(dist / 40, 0, 1) * crazy 
                        
                        -- Generate unpredictable 3D sine-wave offsets
                        local curveX = math.sin(tick() * 10) * curveMultiplier
                        local curveY = math.cos(tick() * 13) * (curveMultiplier * 0.75)
                        local curveZ = math.sin(tick() * 8) * curveMultiplier
                        
                        local dynamicTargetPos = tPos + Vector3.new(curveX, curveY, curveZ)
                        
                        -- As distance closes, aggressively ramp up the BodyPosition Power to strike hard
                        bp.P = 5000 + ((100 / math.max(dist, 1)) * 3000)
                        bp.Position = dynamicTargetPos
                        
                        -- Always face the curve trajectory
    ...
    cinematicToggle = homingMissileModule:CreateToggle({ Name = 'Cinematic Camera', Default = true, Function = function() end, Tooltip = 'Hijacks your camera to follow the missile.'})
    crazySlider = homingMissileModule:CreateSlider({ Name = 'Crazy Factor', Min = 0, Max = 100, Default = 40, Function = function() end, Tooltip = 'More = Unpredictable flight path. 0 = Straight line.' })
...
run(function()
    ...
    modelLoaderModule = vape.Categories['BABFT Tools']:CreateModule({
        ...
        Function = function(callback)
            if not callback then return end
            task.spawn(function()
                if not mCachedData or not mCachedData.blocks then
                ...
                local consumedTracker = {}
                local totalNeeded = #mCachedData.blocks
                ...
                for i, blockData in ipairs(mCachedData.blocks) do
                    ...
                    local targetColor = blockData.color
                    ...
                    task.spawn(function()
                        ...
                        if spawnedBlock and modelLoaderModule.Enabled then
                            if checkIsScalable(currentBlockType) then
                                task.spawn(function() scaleRF:InvokeServer(spawnedBlock, targetSize, absoluteTargetCFrame) end)
                            end
                            table.insert(paintArgs, {spawnedBlock, targetColor})
                        end
                ...
                if paintTool and #paintArgs > 0 then
                    notify('Painting', 'Applying colors...', 5, 'info')
                    task.spawn(function() paintTool:WaitForChild("RF"):InvokeServer(paintArgs) end)
                end
                    ...
                    for x = minX, maxX + step, step do
                        for y = minY, maxY + step, step do
                            for z = minZ, maxZ + step, step do
                                ...
                                local parts = workspace:GetPartBoundsInBox(pos, stepVec, overlapParams)
                                if #parts > 0 then
                                    local rx = math.floor((x - minX) / step + 0.5) * step + minX
...
run(function()
    ...
    local gearModule
    local typeDrop, masterScaleSlider, radSlider, holeSlider, heightSlider
    ...
    local shaftRadSlider, threadDepthSlider, threadThickSlider, turnsSlider
    local uprightToggle, colorDrop, previewToggle
    local offX, offY, offZ
    local blockDropdown, speedSlider, maxModeToggle
    
    local previewModelName = "Advanced_Mechanism_Preview"
    ...
    -- =========================================================================
    -- UNIVERSAL MECHANICAL TRANSFORM ENGINE
    -- =========================================================================
    local function GenerateMechanismData()
        local parts = {}
        ...
        local function getColor(radiusPercent, anglePercent)
            ...
            elseif cStyle == "Brass / Gold" then
                return Color3.new(0.8 - (radiusPercent*0.1), 0.6 - (radiusPercent*0.1), 0.2)
        ...
        -- -----------------------------------------------------------------
        -- GEAR GENERATION MODE
        -- -----------------------------------------------------------------
        if mType == "Standard / Helical Gear" then
            local tCount = teethSlider.Value
            local tDepth = depthSlider.Value * mScale
            local tWidth = widthSlider.Value * mScale
            local twist = math.rad(twistSlider.Value)
            local taper = taperSlider.Value / 100
            local bStyle = bodyDrop.Value
            local spokes = spokeSlider.Value
            local sThick = spokeThickSlider.Value * mScale
            ...
            for l = 1, layers do
                ...
                elseif bStyle == "Spokes" then
                    local rimThick = math.max(2 * mScale, curR * 0.15)
                    local actualRimR = math.max(0.1, curR - rimThick/2)
                    local hubR = math.max(1 * mScale, curHole + (2 * mScale))
                    
                    local hubWidth = hubR - curHole
                    if hubWidth > 0.1 then
        ...
        -- -----------------------------------------------------------------
        -- HIGH-FIDELITY CORKSCREW / SCREW GENERATION MODE
        -- -----------------------------------------------------------------
        elseif mType == "Corkscrew / Precision Screw" then
            ...
            local coreLayerH = H / coreLayers
            local coreSegs = math.max(8, math.floor(shaftRad * 1.5))
            
            for l = 1, coreLayers do
                local yPos = -H/2 + (l - 0.5) * coreLayerH
                for i = 1, coreSegs do
                    local angPercent = i / coreSegs
                    local angle = angPercent * math.pi * 2
                    local w = 2 * shaftRad * math.tan(math.pi / coreSegs)
                    local cf = baseOffset * CFrame.Angles(0, angle, 0) * CFrame.new(0, yPos, -shaftRad + (shaftRad/2))
                    table.insert(parts, {CFrame = cf, Size = Vector3.new(w, coreLayerH, shaftRad), Color = getColor(0.3, angPercent)})
                end
            end
    ...
    local function LiveUpdatePreview()
        ...
        local surfaceY = plotZone.Size.Y / 2
        local calcY = (radSlider.Value * masterScaleSlider.Value) + 10
        local isUpright = uprightToggle.Enabled
        local pivotCFrame = plotZone.CFrame * CFrame.new(0, surfaceY + (isUpright and calcY or (heightSlider.Value * masterScaleSlider.Value)/2 + 5), 0)
        
        primary.CFrame = pivotCFrame
        primary.Parent = previewModel
        previewModel.PrimaryPart = primary

        local partsData = GenerateMechanismData()
    ...
    gearModule = vape.Categories['BABFT Tools']:CreateModule({
        Name = 'Advanced Gears / Screws',
        Tooltip = 'Generates highly complex mechanical gears, corkscrews, and structural augers.',
        Function = function(callback)
            if not callback then return end
            ...
            task.spawn(function()
                ...
                notify('Mechanical Engine', 'Spawning ' .. totalNeeded .. ' precision parts...', 5, 'info')
                pcall(function() workspace:WaitForChild("InstaLoadFunction", 1):InvokeServer() end)
                ...
                local calcY = (radSlider.Value * masterScaleSlider.Value) + 10
                local pivotOffset = CFrame.new(0, surfaceY + (uprightToggle.Enabled and calcY or (heightSlider.Value * masterScaleSlider.Value)/2 + 2), 0)
                ...
                if paintTool and #paintArgs > 0 then
                    notify('Painting', 'Applying Mechanical Surface Finishes...', 5, 'info')
                    task.spawn(function() paintTool:WaitForChild("RF"):InvokeServer(paintArgs) end)
                end

                notify('Mechanical Engine', '✅ Engineering Process Complete!', 5, 'info')
                gearModule:Toggle()
            end)
        end
    })
    ...
    typeDrop = gearModule:CreateDropdown({
        ...
        Function = function(val)
            local isGear = (val == 'Standard / Helical Gear')
            
            -- Toggle Gear UI Elements
            radSlider.Object.Visible = isGear
            holeSlider.Object.Visible = isGear
            teethSlider.Object.Visible = isGear
            depthSlider.Object.Visible = isGear
            widthSlider.Object.Visible = isGear
            twistSlider.Object.Visible = isGear
            taperSlider.Object.Visible = isGear
            bodyDrop.Object.Visible = isGear
            if not isGear then
                spokeSlider.Object.Visible = false
                spokeThickSlider.Object.Visible = false
            else
                spokeSlider.Object.Visible = (bodyDrop.Value == 'Spokes')
                spokeThickSlider.Object.Visible = (bodyDrop.Value == 'Spokes')
            end
            
            -- Toggle Corkscrew UI Elements
            shaftRadSlider.Object.Visible = not isGear
            threadDepthSlider.Object.Visible = not isGear
            threadThickSlider.Object.Visible = not isGear
            turnsSlider.Object.Visible = not isGear
            
            LiveUpdatePreview()
        end
    })

    previewToggle = gearModule:CreateToggle({
        Name = 'Spin Preview Mode',
        Default = false,
        Function = function(val)
            LiveUpdatePreview()
        end
    ...
    depthSlider = gearModule:CreateSlider({ Name = 'Tooth Profile Depth', Min = 1, Max = 40, Default = 5, Function = LiveUpdatePreview })
    widthSlider = gearModule:CreateSlider({ Name = 'Tooth Face Width', Min = 1, Max = 25, Default = 4, Function = LiveUpdatePreview })
    twistSlider = gearModule:CreateSlider({ Name = 'Helical Twist Profile', Min = -180, Max = 180, Default = 0, Function = LiveUpdatePreview })
    taperSlider = gearModule:CreateSlider({ Name = 'Bevel Angle % (Cone Taper)', Min = 0, Max = 90, Default = 0, Function = LiveUpdatePreview })

    bodyDrop = gearModule:CreateDropdown({ 
        Name = 'Internal Webbing Style', 
        List = {'Solid', 'Spokes'}, 
        Function = function(val)
            if typeDrop.Value == 'Standard / Helical Gear' then
    ...
    offX = gearModule:CreateSlider({ Name = 'Offset Adjust X', Min = -500, Max = 500, Default = 0, Function = LiveUpdatePreview })
    offY = gearModule:CreateSlider({ Name = 'Offset Adjust Y', Min = -500, Max = 500, Default = 0, Function = LiveUpdatePreview })
    offZ = gearModule:CreateSlider({ Name = 'Offset Adjust Z', Min = -500, Max = 500, Default = 0, Function = LiveUpdatePreview })

    blockDropdown = gearModule:CreateDropdown({ Name = 'Material Block Type', List = getAvailableBlocks(), Function = function() end})
